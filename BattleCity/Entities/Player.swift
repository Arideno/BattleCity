...
final class Player: SKSpriteNode {
    ...
    func moveDown() {
        guard !hasActions() else { return }
        direction = .down
        run(SKAction.sequence([SKAction.rotate(toAngle: .pi, duration: 0), SKAction.move(by: CGVector(dx: 0, dy: -Constants.cellSize.height), duration: 0.2)]))
    }

    func moveLeft() {
        guard !hasActions() else { return }
        direction = .left
        run(SKAction.sequence([SKAction.rotate(toAngle: .pi / 2, duration: 0), SKAction.move(by: CGVector(dx: -Constants.cellSize.width, dy: 0), duration: 0.2)]))
    }
